...
async def main():
  ...
  while True:
    ...
    match selection:
      case "1":
        courselist = await _api.get_course_list(coursetitles=[""],centerIds=[23,33,54])
      ...
      case "3":
        access_token = await _api.login(user=my_secrets.user,pwd=my_secrets.pwd)
        courselist = await _api.get_course_list(coursetitles=["BODYPUMP® 55'"],centerIds=[23,33,54])
      ...
      case "5":
        ...
        access_token =await _api.login(user=my_secrets.user,pwd=my_secrets.pwd) # requires a login
        ...
        try:
            print('1')
            await _api.book_course(access_token=access_token,course_id=courselist.courses_bookable[int(select)].course_id_tac)
            print('2')
            bookings = await _api.get_bookings()
            print('3')
            print(bookings.courses)
      ...
      case "6":
          ...
          try:
            await _api.cancel_course(booking_id=bookings.courses[int(select)].booking_id_tac)
            bookings = await _api.get_bookings()
            print(bookings.courses)
          except:
            print("Invalid selection")
      ...
      case "8":
        ...
        try:
          selected_centers = [int(id) for id in selection.split(",")]
          print(selected_centers)
          await _api.get_course_list(centerIds=selected_centers)
        except:
          print("Invalid selection")
          pass
        ...
        selection = input("Enter courses seperated by commas: ")
        try:
          selected_courses = [int(id) for id in selection.split(",")]
          print(selected_courses)
          selected_course_names = [coursetitles_sorted[i] for i in selected_courses]
          print(selected_course_names)
          # This would be the regular home assistant call to update the (10) course devices:
          await _api.get_course_list(coursetitles=selected_course_names,centerIds=selected_centers,take=30)
        except:
          print("Invalid selection")
          pass
