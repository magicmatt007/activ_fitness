...
class Api:
    ...
    # Step 10: Migros Login GET csrf
      ...
      r = await self.session.get(url,allow_redirects=False,ssl_context=self.ssl_context)
      mylogger.debug(f"\nStep 10 {url}")
    ...
    # Step 20: Migros Login POST credentials
      ...
      'username': user,
      'password': pwd}
      r = await self.session.post(url,data=payload,allow_redirects=False,ssl_context=self.ssl_context)
      mylogger.debug(r)
    ...
    # Step 30: Migros OAuth2 authorize: get code and state for client_id and code_challenge
      ...
      r = await self.session.get(url,params=payload, allow_redirects=False,ssl_context=self.ssl_context)
      mylogger.debug(r)
      try:
    ...
    async def get_course_list(self,coursetitles:list[str]=[""], centerIds: list[int]=[33,23,54],take:int=10):  # "BODYPUMP® 55'"
      """
      ...
      for coursetitle in coursetitles:
        coursetitles_lst.extend(list(map(lambda x: {"centerId":x, "coursetitle": coursetitle},centerIds)))
      payload = json.dumps({
        "language": "de",
        "skip": 0,
        "take": take,
      ...
      r = await self.session.post(url,data=payload,headers=headers,allow_redirects=False,ssl_context=self.ssl_context)
    ...
    async def get_bookings(self):
      ...
      r = await self.session.get(url,headers = headers, allow_redirects=False,ssl=self.ssl_context)
    ...
    async def get_checkins(self,from_="2022-01-01",to_="2022-11-31"):
      ...
      r = await self.session.get(url,params=payload, allow_redirects=True,ssl_context=self.ssl_context)
      mylogger.debug(r)
    ...
    async def book_course(self,course_id):
      ...
      r = await self.session.post(url,headers = headers, json = payload, allow_redirects=False,ssl=self.ssl_context)
      mylogger.debug(r)
    ...
    async def cancel_course(self,booking_id):
      ...
      r = await self.session.delete(url,headers=headers,json=payload, allow_redirects=False,ssl=self.ssl_context)
      mylogger.debug(r)
